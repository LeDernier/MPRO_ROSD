...
function main_iterative()
    for dataSetName in ["iris", "seeds", "wine", "new_data_1", "new_data_2"]
        ...
        for D in 2:4
            println("\tD = ", D)
            println("\t\tUnivarié")
            println("\t\t\t- Iterative benders (FeS)")
            testIterative(X_train, Y_train, X_test, Y_test, D, classes, time_limit=time_limit, isExact=true)
...
function testIterative(X_train, Y_train, X_test, Y_test, D, classes; time_limit::Int=-1, isExact::Bool=false)
    ...
    for gamma in 0:0.2:0.8
        ...
        T, obj, resolution_time, gap, iterationCount = iteratively_build_tree_benders(clusters, D, X_train, Y_train, classes, time_limit=time_limit)
        if gap == -1
            print("???%\t")
        else
